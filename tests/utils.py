...
def utils_transform_provider_data(dataset: pd.DataFrame, provider: str, data_type: str) -> pd.DataFrame:
    generic_id = f"provider_{data_type}_id"
    specific_id = f"{provider}_{data_type}_id"

    dataset.rename(
        {
            generic_id: specific_id
        },
        axis=1,
        inplace=True
    )
...
def utils_create_syncables(dataset: pd.DataFrame, data_type: str) -> list[SyncableContent]:
    grouped = dataset.groupby("data_provider")
    syncables = [
        SyncableContent(
            provider=p, 
            data=utils_transform_provider_data(dataset.loc[dataset.index.isin(d), ], p, data_type),
            data_type=data_type
        ) 
