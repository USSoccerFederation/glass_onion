...
def test_init_competition_context():
    engine = TeamSyncEngine(
        content=[
            TeamSyncableContent("provider_a", pd.DataFrame(columns = ["provider_a_team_id"]))
        ],
...
def test_synchronize_using_layer(
    ...
    assert set(["team_name", "provider_a_team_id", "provider_b_team_id"]) == set(result.data.columns)
